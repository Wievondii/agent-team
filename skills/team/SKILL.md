...
| 审查员 | 按需拉起，完成即结束 | 审查代码规范、架构、安全 | 只读 + 写日志 | opus | `agent-team-review-log.md` |
...
2. **删除并重建私有日志**：三个私有日志全部删除，从模板重新创建
3. 在共享日志末尾追加新的轮次章节
...
- **⚠️ 有条件通过**：🟡 建议不超过 3 个 → 放行，记录在案。超过 3 个 → 打回
